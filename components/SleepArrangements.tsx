...
function Lightbox({
  ...
  return createPortal(
    <div
      ...
      style={{ background: 'rgba(8,8,7,0.97)', zIndex: 9999 }}
    ...
    >
      ...
      >
        ...
        <button
          ...
          onMouseEnter={(e) => {
            ;(e.currentTarget as HTMLButtonElement).style.background = 'rgba(212,175,55,0.28)'
          }}
          onMouseLeave={(e) => {
      ...
      {/* ── Image area ── */}
      <div className="flex-1 relative flex items-center justify-center min-h-0 px-14 md:px-20 py-4">
        {/* Prev */}
        <button
          type="button"
          onClick={onPrev}
      ...
      {/* ── Bottom bar ── */}
      <div
        className="flex-shrink-0 px-5 py-4 text-center"
        style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}
      >
        <p className="font-serif text-white text-lg font-semibold">{room.name}</p>
        <p className="text-sm font-sans mt-1" style={{ color: 'rgba(212,175,55,0.7)' }}>
          {room.beds}
        </p>
          ...
          {BEDROOMS.map((_, i) => (
            <button
              ...
              onClick={() => {
                ...
                if (i > index) { for (let j = 0; j < i - index; j++) onNext() }
              }}
              className="rounded-full transition-all"
              style={{
                width: i === index ? '20px' : '6px',
                height: '6px',
                background: i === index ? '#D4AF37' : 'rgba(255,255,255,0.2)',
              }}
            />
          ))}
        </div>
...
export default function SleepArrangements() {
  ...
  // Scroll lock — only engage on first open, not on bedroom navigation
  useEffect(() => {
    if (activeIndex !== null) {
            ...
            {BEDROOMS.map((room, i) => (
              ...
              >
                ...
                >
                  ...
                  <div className="hidden dark:block absolute inset-0 rounded-2xl pointer-events-none" style={{ background: 'linear-gradient(160deg,#1a1810,#111008)', zIndex: 0 }} />
                  ...
                  <div className="relative w-full overflow-hidden" style={{ aspectRatio: '4/3' }}>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/card:opacity-100 transition-opacity duration-300"
                      ...
                      <span className="material-icons text-white" style={{ fontSize: '32px' }}>zoom_in</span>
                    </div>
                  </div>
